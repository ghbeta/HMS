...
public class ForumThread extends Model {
    ...
    public static List<ForumThread> findByLecture(String semester,Lecture lecture){
        return getServer(semester).find(ForumThread.class).where().eq("lecture.courseName", lecture.courseName).orderBy("lastupdatetimestamp,lastupdatetimestamp asc").findList();
    }
    ...
    public static List<ForumThread> findByLectureByStudent(String semester,Lecture lecture,Semesteruser semesteruser){
        return getServer(semester).find(ForumThread.class).where().eq("lecture.courseName",lecture.courseName).eq("creator.id",semesteruser.id).orderBy("lastupdatetimestamp,lastupdatetimestamp asc").findList();
    }
}
